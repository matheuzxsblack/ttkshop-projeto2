(async function () {
  ...
  try {
    if (window.TTK_STOREFRONT_READY) await window.TTK_STOREFRONT_READY;
  } catch (eReady) {}
  var SF = window.TTK_STOREFRONT || {};
  if (SF.slug) window.TTK_STORE = SF.slug;
  ...
  function ttkProdId() {
    return String(window.TTK_STORE || "jaqueta").toLowerCase();
  }
  ...
  var KIT_PICK = Math.max(1, Math.min(8, parseInt(SF.optionPickCount, 10) || 1));
  var KIT_MODE = KIT_PICK > 1;
  var OPTION_LABEL = String(SF.optionLabel || "Cor");
  var SIZE_LIST = Array.isArray(SF.sizes)
    ? SF.sizes.map(function (s) { return String(s || "").trim(); }).filter(Boolean)
    : [];
  var SHOW_SIZES = SIZE_LIST.length > 0;
  var pickOrder = []; /* { color, size, img } */

  function selectedColorOpt() {
    return document.querySelector("#sku-grid .sku-opt.selected, #sku-grid .sku-opt.has-qty");
  }
  function selectedSizeOpt() {
    return document.querySelector("#size-grid .size-opt.selected");
  }

  function htmlEscEarly(s) {
    return String(s || "")
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
  ...
  function kitLabel() {
    return pickOrder
      .map(function (p) {
        return SHOW_SIZES && p.size ? p.color + ", " + p.size : p.color;
      })
      .join(" + ");
  }
  ...
  function syncKitUI() {
    ...
    var counts = {};
    pickOrder.forEach(function (p) {
      counts[p.color] = (counts[p.color] || 0) + 1;
    });
    document.querySelectorAll("#sku-grid .sku-opt").forEach(function (o) {
      var q = counts[o.dataset.color] || 0;
      o.classList.toggle("has-qty", q > 0);
      o.classList.toggle("selected", KIT_MODE ? q > 0 : o.classList.contains("selected"));
    ...
    if (chips) {
      if (!KIT_MODE || !pickOrder.length) {
        ...
        chips.innerHTML = pickOrder
          .map(function (p, idx) {
            var img = p.img
              ? '<img src="' + htmlEscEarly(p.img) + '" alt="" />'
              : "";
            var lab = SHOW_SIZES && p.size ? p.color + " · " + p.size : p.color;
            return (
              ...
              idx +
              '" aria-label="Remover">×</button></span>'
            );
          })
          .join("");
      }
    }
    var lbl = document.getElementById("pick-count-label");
    if (lbl && KIT_MODE) lbl.textContent = n + " de " + KIT_PICK;
    var bar = document.getElementById("pick-bar-fill");
    if (bar && KIT_MODE) {
      bar.style.width = Math.min(100, Math.round((n / KIT_PICK) * 100)) + "%";
    }
    if (optHeadV) {
      if (KIT_MODE) {
        optHeadV.textContent = n ? pickOrder.map(function (p) { return p.color; }).join(", ") : "Selecione";
      } else {
        var sel = document.querySelector("#sku-grid .sku-opt.selected");
        optHeadV.textContent = sel ? sel.dataset.color : "Selecione";
      }
    }
    if (sizeHint) {
      var sz = selectedSizeOpt();
      sizeHint.textContent = sz ? sz.dataset.size : "Selecione";
    }
    var last = pickOrder.length ? pickOrder[pickOrder.length - 1] : null;
    if (last && last.img) {
      var thumb = document.getElementById("sku-thumb");
      if (thumb) thumb.src = last.img;
    }
  }

  function resetKitPick() {
    pickOrder = [];
    document.querySelectorAll("#sku-grid .sku-opt").forEach(function (o) {
      o.classList.remove("has-qty");
      if (KIT_MODE) o.classList.remove("selected", "selected2");
      var badge = o.querySelector(".sku-qty-badge");
      if (badge) {
        badge.hidden = true;
        badge.textContent = "0";
  ...
  function addKitPick(opt) {
    if (pickOrder.length >= KIT_PICK) {
      showToast("Já escolheu as " + KIT_PICK + " opções do kit.");
      return;
    }
    var size = selectedSizeOpt();
    pickOrder.push({
      color: opt.dataset.color,
      size: size ? size.dataset.size : "",
      img: opt.dataset.img || "",
      price: Number(opt.dataset.price) > 0 ? Number(opt.dataset.price) : 0,
    });
    if (opt.dataset.img) {
      var thumb = document.getElementById("sku-thumb");
  ...
  function clearExtras() {
    if (!extraList) return;
    extraList.querySelectorAll(".extra-row").forEach(function (row) {
  ...
  function openSku() {
    ttkFunnel("product", { product_id: ttkProdId() });
    /* upsells sempre começam desligados — a pessoa marca se quiser */
    clearExtras();
    if (KIT_MODE) resetKitPick();
  ...
  document.getElementById("sku-grid").addEventListener("click", function (e) {
    ...
    if (badge && KIT_MODE) {
      var ownerB = badge.closest(".sku-opt");
      if (ownerB) {
        var idx = -1;
        for (var bi = pickOrder.length - 1; bi >= 0; bi--) {
          if (pickOrder[bi].color === ownerB.dataset.color) {
            idx = bi;
            break;
          }
        }
        if (idx !== -1) removeKitAt(idx);
      }
      e.preventDefault();
      e.stopPropagation();
      return;
    ...
    if (optHeadV) optHeadV.textContent = opt.dataset.color || "Selecione";
    updateSkuPrice();
    handleVariantPick();
  });
  ...
  if (sizeGridEl) {
    sizeGridEl.addEventListener("click", function (e) {
      ...
      if (KIT_MODE && pickOrder.length) {
        pickOrder[pickOrder.length - 1].size = opt.dataset.size;
        syncKitUI();
      }
      handleVariantPick();
    });
  ...
  if (pickChipsEl) {
    pickChipsEl.addEventListener("click", function (e) {
      ...
      if (!btn) return;
      removeKitAt(parseInt(btn.getAttribute("data-chip"), 10));
    });
  }

  /* upsells (checkbox) — só troca com clique direto; sem foto nesta tela */
  if (extraList) {
    extraList.addEventListener("click", function (e) {
      var row = e.target.closest(".extra-row");
      if (!row || !extraList.contains(row)) return;
      e.stopPropagation();
      var on = !row.classList.contains("checked");
      row.classList.toggle("checked", on);
      row.setAttribute("aria-checked", on ? "true" : "false");
    });
    extraList.addEventListener("keydown", function (e) {
  ...
  var cartItems = []; /* { label, img, qty, price, extra, upsellTitle, upsellSub } */
  var editingIndex = null; /* item do carrinho em troca de variante */
  var PRICE = Number(SF.price) > 0 ? Number(SF.price) : 34.99;
  var OLD_PRICE = Number(SF.oldPrice) > PRICE ? Number(SF.oldPrice) : Math.round((PRICE / 0.32) * 100) / 100;
  var UPSELLS = (function () {
  ...
  var PRODUCT_TITLE = String(SF.title || "Oferta");
  var PRODUCT_TITLE_SHORT = PRODUCT_TITLE.length > 36 ? PRODUCT_TITLE.slice(0, 34) + "…" : PRODUCT_TITLE;
  var OLD_PRICE_LABEL = "R$ " + OLD_PRICE.toFixed(2).replace(".", ",");
  var OFF_LABEL =
    OLD_PRICE > 0
      ? "-" + Math.max(1, Math.min(95, Math.round((1 - PRICE / OLD_PRICE) * 100))) + "%"
      : "-68%";
  ...
  function optionUnitPrice(el) {
    var p = Number(el && (el.dataset.price || el.getAttribute("data-price")));
    return p > 0 ? p : PRICE;
  }

  function kitLinePrice() {
    var hasCustom = pickOrder.some(function (p) {
      return Number(p.price) > 0;
    });
    if (!hasCustom) return PRICE;
    return pickOrder.reduce(function (sum, p) {
      return sum + (Number(p.price) > 0 ? Number(p.price) : PRICE);
    }, 0);
  }

  function sheetUnitPrice() {
    if (KIT_MODE && editingIndex === null && pickOrder.length) return kitLinePrice();
    return optionUnitPrice(selectedColorOpt());
  }

  function updateSkuPrice() {
    var p = sheetUnitPrice();
    var root = document.querySelector(".sku-price-info");
    if (!root || !(p > 0)) return;
    var parts = p.toFixed(2).split(".");
    root.querySelectorAll(".price-int").forEach(function (el) {
      el.textContent = parts[0] || "0";
    });
    root.querySelectorAll(".price-cents").forEach(function (el) {
      el.textContent = "," + (parts[1] || "00");
    });
    var old = OLD_PRICE;
    if (PRICE > 0 && OLD_PRICE > PRICE && Math.abs(p - PRICE) > 0.001) {
      old = Math.round(OLD_PRICE * (p / PRICE) * 100) / 100;
    }
    if (!(old > p)) old = Math.round((p / 0.32) * 100) / 100;
    var oldEl = root.querySelector(".price-old");
    if (oldEl) oldEl.textContent = "R$ " + old.toFixed(2).replace(".", ",");
    var off = Math.max(1, Math.min(95, Math.round((1 - p / old) * 100)));
    root.querySelectorAll(".discount-badge").forEach(function (el) {
      el.textContent = "-" + off + "%";
    });
  }
  ...
  function selectedUpsells() {
    var out = [];
    if (!extraList) return out;
    extraList.querySelectorAll(".extra-row.checked").forEach(function (row) {
      var i = parseInt(row.getAttribute("data-ui"), 10);
      var u = UPSELLS[i];
      if (u && Number(u.price) > 0) out.push(u);
    });
    return out;
  }

  function selectedUpsellCents() {
    return selectedUpsells().reduce(function (sum, u) {
      return sum + Math.round(Number(u.price) * 100);
    }, 0);
  }
  ...
  function extraDisplayTitle(item, short) {
    var t = String(item.upsellTitle || item.label || "Oferta extra");
    if (short && t.length > 42) return t.slice(0, 40) + "…";
    return t;
  }

  function extraDisplaySub(item) {
    var sub = String(item.upsellSub || "").trim();
    if (sub && sub !== item.upsellTitle) return sub;
    return "";
  }

  function lineImgHtml(item, cls) {
    var src = String(item.img || "").trim();
    if (!src) return "";
    return '<img class="' + cls + '" src="' + htmlEsc(src) + '" alt="" />';
  }
  ...
  document.getElementById("qty-plus").addEventListener("click", function () {
    var extraC = selectedUpsellCents();
    var unitC = Math.round(sheetUnitPrice() * 100);
    var prospective = cartCents() + (qty + 1) * unitC + extraC;
  ...
  function itemHtml(item, i) {
    var extraSub = item.extra ? extraDisplaySub(item) : "";
    var variantBtn =
      item.extra || item.kit
        ? extraSub || item.kit
          ? '<span class="cart-variant static"><span>' + htmlEsc(item.kit ? item.label : extraSub) + "</span></span>"
          : ""
        : '<button class="cart-variant" type="button" data-vi="' + i + '"><span>' + htmlEsc(item.label) + "</span> " + CHEV_DOWN_SVG + "</button>";
    var oldRow = item.extra
      ? ""
      : '<p class="cart-old-row"><span class="price-old">' + OLD_PRICE_LABEL + '</span> <span class="cart-off">' + OFF_LABEL + "</span></p>";
    var title = item.extra ? extraDisplayTitle(item, true) : PRODUCT_TITLE_SHORT;
    return (
      '<div class="cart-item">' +
      '<span class="cart-check checked" aria-hidden="true"></span>' +
      lineImgHtml(item, "cart-item-img") +
      '<div class="cart-item-info">' +
      '<p class="cart-item-title' + (item.extra ? " extra-line" : "") + '">' + htmlEsc(title) + "</p>" +
      variantBtn +
  ...
  function variantLabel(colorOpt, sizeOpt) {
    if (colorOpt && sizeOpt && SHOW_SIZES) return colorOpt.dataset.color + ", " + sizeOpt.dataset.size;
    if (colorOpt) return colorOpt.dataset.color;
    return "Padrão";
  }
  ...
  function addToCart(mode) {
    ...
    var size = selectedSizeOpt();
    var kitOn = KIT_MODE && editingIndex === null;

    if (kitOn) {
      if (pickOrder.length < KIT_PICK) {
        showToast("Escolha " + KIT_PICK + " " + OPTION_LABEL.toLowerCase() + " para o kit.");
        return;
      }
      if (SHOW_SIZES) {
        var kitSize = selectedSizeOpt();
        if (kitSize) {
          pickOrder.forEach(function (p) {
            if (!p.size) p.size = kitSize.dataset.size;
          });
        }
        if (pickOrder.some(function (p) { return !p.size; })) {
    ...
    /* ajusta quantidade/extras para o pedido não passar de R$ 200 */
    var budgetC = MAX_C - cartCents();
    var picked = selectedUpsells();
    var extraC = picked.reduce(function (sum, u) {
      return sum + Math.round(Number(u.price) * 100);
    }, 0);
    var addQty = qty;
    var unitPrice = kitOn ? kitLinePrice() : optionUnitPrice(sel);
    var unitC = Math.round(unitPrice * 100) || PRICE_C;

    if (addQty * unitC + extraC > budgetC) {
      while (picked.length && addQty * unitC + extraC > budgetC) {
        extraC -= Math.round(Number(picked.pop().price) * 100);
      }
      if (addQty * unitC + extraC > budgetC) {
        addQty = Math.floor((budgetC - extraC) / unitC);
      }
      if (addQty < 1) {
        warnLimit();
        return;
    ...
    var label;
    var img = (SF.gallery && SF.gallery[0]) || "";
    if (kitOn) {
      label = kitLabel();
      img = (pickOrder[0] && pickOrder[0].img) || img;
    } else {
      label = size ? "Opção, " + size.dataset.size : "Padrão";
      if (sel) {
    ...
    picked.forEach(function (u) {
      var key = u.title + "|" + Number(u.price);
      var existingExtra = cartItems.find(function (it) {
        return it.extra && it.upsellKey === key;
      });
      if (existingExtra) {
        existingExtra.qty += 1;
      } else {
  ...
  function openSkuForEdit(i) {
    var item = cartItems[i];
    if (!item || item.kit || item.extra) return;
    editingIndex = i;
  ...
  function handleVariantPick() {
    ...
    if (otherIdx !== -1) {
      ...
      item.label = label;
      item.img = img;
      item.price = optionUnitPrice(sel);
    }
  ...
  fetch(apiUrl("/api/checkout-mode?store=" + encodeURIComponent(ttkProdId())))
  ...
  function coItemHtml(item, i) {
    var oldRow = item.extra
      ? ""
      : '<p class="co-item-old"><span class="price-old">' + OLD_PRICE_LABEL + '</span> <span class="cart-off">' + OFF_LABEL + "</span></p>";
    var title = item.extra ? extraDisplayTitle(item, true) : PRODUCT_TITLE_SHORT;
    var sub = item.extra ? extraDisplaySub(item) : item.label;
    return (
      '<div class="co-item">' +
      lineImgHtml(item, "co-item-img") +
      '<div class="co-item-info">' +
      '<p class="co-item-title' + (item.extra ? " extra-line" : "") + '">' + htmlEsc(title) + "</p>" +
      (sub ? '<p class="co-item-variant">' + htmlEsc(sub) + "</p>" : "") +
  ...
  function scItemHtml(item, i) {
    var title = item.extra ? extraDisplayTitle(item, false) : PRODUCT_TITLE;
    var extraSub = item.extra ? extraDisplaySub(item) : item.label;
    var oldRow = item.extra
      ? ""
      : '<span class="sc-item-old">' + OLD_PRICE_LABEL + '</span> <span class="sc-item-off">' + OFF_LABEL + "</span>";
    return (
      '<div class="sc-item">' +
      lineImgHtml(item, "sc-item-img") +
      '<div class="sc-item-info">' +
      '<p class="sc-item-title">' +
      htmlEsc(title) +
      (extraSub ? ' <span class="sc-variant">(' + htmlEsc(extraSub) + ")</span>" : "") +
      "</p>" +
  ...
  function createPixCharge(total) {
    ...
    return fetch(apiUrl("/api/pix"), {
      ...
      body: JSON.stringify(Object.assign({
        amount: cents,
        origem: SIMULATE_MODE ? "simular" : String(window.TTK_STORE || SF.slug || "loja") + "-ttkshop",
        ...
        items_detail: cartItems.map(function (it) {
          return { variante: PRODUCT_TITLE.split(" ")[0] + " " + it.label, qtd: it.qty };
        }),
  ...
  function sucItemHtml(item) {
    ...
    return (
      '<div class="suc-item">' +
      lineImgHtml(item, "suc-item-img") +
      '<div class="suc-item-info">' +
      "<p>" + htmlEsc(title) + "</p>" +
      "<span>" + (sub ? htmlEsc(sub) + " · " : "") + item.qty + " un.</span>" +
      "</div>" +
  ...
  function firePurchasePixel(order, attempt) {
    ...
    function doTrack() {
      ...
      var contents = (order.items || []).map(function (it, i) {
        return {
          content_id: String(window.TTK_STORE || "loja") + "-" + String(it.label || i).replace(/\s+/g, "-").toLowerCase(),
          content_type: "product",
          content_name: it.extra
            ? extraDisplayTitle(it, false)
            : PRODUCT_TITLE + " " + (it.label || ""),
      ...
      var payload = {
        contents: contents.length
          ? contents
          : [{ content_id: String(window.TTK_STORE || "loja"), content_type: "product", content_name: PRODUCT_TITLE, quantity: 1, price: value }],
        content_type: "product",
        currency: "BRL",
        value: value,
      };
