(function () {
  ...
  function ttkProdId() {
    return String(window.TTK_STORE || "coberdrom").toLowerCase();
  }
  ...
  function itemHtml(item, i) {
    ...
    var oldRow = item.extra
      ? ""
      : '<p class="cart-old-row"><span class="price-old">R$ 118,00</span> <span class="cart-off">-68%</span></p>';
    return (
      '<div class="cart-item">' +
  ...
  function addToCart(mode) {
    ...
    if (!sel) {
      var firstColor = document.querySelector("#sku-grid .sku-opt");
      if (firstColor) {
  ...
  fetch(apiUrl("/api/checkout-mode?store=coberdrom"))
  ...
  function createPixCharge(total) {
    ...
    return fetch(apiUrl("/api/pix"), {
      ...
      body: JSON.stringify(Object.assign({
        amount: cents,
        origem: SIMULATE_MODE ? "simular" : "coberdrom-ttkshop",
