(function () {
  ...
  /* leve +1 cor aleatória (checkbox) */
  extraRow.addEventListener("click", function () {
    extraRow.classList.toggle("checked");
    extraRow.setAttribute("aria-checked", extraRow.classList.contains("checked") ? "true" : "false");
  });
  ...
  document.getElementById("qty-plus").addEventListener("click", function () {
    var extraC = extraRow.classList.contains("checked") ? EXTRA_C : 0;
  ...
  function itemHtml(item, i) {
    ...
    return (
      ...
      '<p class="cart-item-title">' + (item.extra ? "Cor surpresa — Jaqueta Puffer Prem..." : "Jaqueta Feminina Puffer Forrada Pre...") + "</p>" +
      variantBtn +
  ...
  function addToCart(mode) {
    ...
    var addExtra = extraRow.classList.contains("checked");
    var addQty = qty;
  ...
  function coItemHtml(item, i) {
    ...
    return (
      ...
      '<p class="co-item-title">' + (item.extra ? "Cor surpresa — Jaqueta Puffer Prem..." : "Jaqueta Feminina Puffer Forrada Pre...") + "</p>" +
  ...
  document.getElementById("btn-save-address").addEventListener("click", function () {
    ...
    if (cpf.replace(/\D/g, "").length !== 11) {
      showToast("CPF incompleto. Digite os 11 dígitos.");
      cpfInput.focus();
      return;
    }
  ...
  function startPixPoll(txId) {
    ...
    pixPollId = setInterval(function () {
      fetch("/api/pix/" + encodeURIComponent(txId))
        .then(function (r) {
          return r.json();
        })
        .then(function (json) {
